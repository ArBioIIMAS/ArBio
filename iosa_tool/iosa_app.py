...
def processing_manual(file_uploaded, filename):
    ...
    canvas_result = st_canvas(
        ...
        background_image= Image.open(file_uploaded).convert("RGB"),
        height=400,
...
def processing_automatic_segmentation(filename):
    image_np = st.session_state["imagen_cargada"]   # recuperar
    ...
    pos2 = np.array([[x1_l2, y1_l2],[x2_l2, y2_l2]])
...
def main():
    ...
    if st.session_state.segmentation == "automatic":
        ...
        if file_uploaded is not None:
            ...
            processing_automatic_segmentation(file_uploaded.name)
        else:
            print("nada cargado")
...
#************************** Dashboard ***************************#
st.title("MorphOtolith: a tool for segmenting otolith images")
st.divider()
my_logo = add_logo()
