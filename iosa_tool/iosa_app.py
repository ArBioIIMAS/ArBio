...
def processing_automatic_segmentation(file_uploaded, imagen_original):
    ...
    display_width = 400
    ratio = img_width / display_width

    display_height = int(img_height / ratio)

    # Abrir imagen
    image_coors = imagen_original
    small_img = image_coors.resize((display_width, display_height))    # Redimensionar
    ...
    print(
        ...
        "small_img_array:", small_img_array.shape
    )
    ...
    canvas_result = st_canvas(
        ...
        # OJO: width = columnas
        width=small_img_array.shape[1],

        # OJO: height = filas
        height=small_img_array.shape[0],
