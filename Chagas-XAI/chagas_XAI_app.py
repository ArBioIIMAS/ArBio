...
def get_model(n_steps, n_features):
    model = TimesNet(
                ...
                num_workers=0,
                device=device)
    # model.load('model.pypots')

    if not os.path.isfile('model.pypots'):
        urllib.request.urlretrieve('https://github.com/ArBioIIMAS/ArBio/raw/refs/heads/main/Chagas-XAI/bs_64_TimesNet_fs_no_aug_no.pypots', 'model.pypots')
    #return tf.keras.models.load_model('model.h5')
    model.load('model.pypots')

    #model = torch.load('model.pypots', weights_only=False)
